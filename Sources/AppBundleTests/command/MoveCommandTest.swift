        ...
        return switch nodeCases {
            ...
            case .tilingContainer(let container):
                switch container.layout {
                    ...
                    case .tabbed:
                        container.orientation == .h
                            ? .h_tabbed(container.children.map(\.layoutDescription))
                            : .v_tabbed(container.children.map(\.layoutDescription))
                    case .niri:
                        .niri(container.children.map(\.layoutDescription))
                }
        }
    }
}
...
enum LayoutDescription: Equatable {
    ...
    case h_tabbed([LayoutDescription])
    case v_tabbed([LayoutDescription])
    case niri([LayoutDescription])
    case window(UInt32)
